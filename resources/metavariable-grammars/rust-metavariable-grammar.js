...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$._literal, $.identifier],
    [$.pattern, $.identifier],
    [$.delim_token_tree, $._literal, $.identifier],
    [$.token_tree, $._literal, $.identifier],
    [$.visibility_modifier, $.identifier],
    [$.parameters, $.identifier],
    [$._statement, $._literal, $.identifier],
    [$._statement, $.identifier],
    [$._statement, $.visibility_modifier],
    [$.declaration_list, $.visibility_modifier],
  ...
  rules: {
    source_file: $ => repeat(field('statements', $._statement)),
    ...
    expression_statement: $ => choice(
      ...
      prec(1, field('expression', $._expression_ending_with_block))
    ),
    ...
    token_tree: $ => choice(
      seq('(', field('tokens', repeat(choice($.grit_metavariable, $._tokens))), ')'),
      seq('[', field('tokens', repeat(choice($.grit_metavariable, $._tokens))), ']'),
      seq('{', field('tokens', repeat(choice($.grit_metavariable, $._tokens))), '}')
    ),
    ...
    declaration_list: $ => seq(
      '{',
      field('statements', repeat($._declaration_statement)),
      '}'
    ),
    ...
    field_declaration_list: $ => seq(
      '{',
      field('declarations', choice($.grit_metavariable, 
      seq(sepBy(',', seq(repeat($.attribute_item), $.field_declaration)), optional(',')))),
      '}'
    ),
    ...
    constrained_type_parameter: $ => seq(
      field('left', choice($.lifetime, $.type_identifier)),
      field('bounds', $.trait_bounds)
    ),
    ...
    parameters: $ => prec(PREC.grit_metavariable, seq(
      '(',
      field('params', sepBy(',', seq(
    ...
    macro_invocation: $ => seq(
      ...
      '!',
      field('argument', alias($.delim_token_tree, $.token_tree))
    ),

    delim_token_tree: $ => choice(
      seq('(', field('tokens', repeat(choice($.grit_metavariable, $._delim_tokens))), ')'),
      seq('[', field('tokens', repeat(choice($.grit_metavariable, $._delim_tokens))), ']'),
      seq('{', field('tokens', repeat(choice($.grit_metavariable, $._delim_tokens))), '}')
    ),
    ...
    range_expression: $ => prec.left(PREC.range, choice(
      seq(field('left', $._expression), choice('..', '...', '..='), field('right', $._expression)),
    ...
    return_expression: $ => choice(
      prec.left(seq('return', field('value', $._expression))),
      prec(-1, 'return'),
    ),

    yield_expression: $ => choice(
      prec.left(seq('yield', field('value', $._expression))),
      prec(-1, 'yield'),
    ),
    ...
    arguments: $ => seq(
      '(',
      field('args', 
        seq(sepBy(',', seq(repeat($.attribute_item), $._expression)),
        optional(','))
      ),
      ')'
    ),
    ...
    match_block: $ => seq(
      '{',
        field('arms', choice( $.grit_metavariable, optional(seq(
          repeat($.match_arm),
    ...
    index_expression: $ => prec(PREC.call, seq(field('expression', $._expression), '[', field('index', $._expression), ']')),
    ...
    tuple_pattern: $ => seq(
      '(',
        field('patterns', seq(sepBy(',', $.pattern),
        optional(','))),
      ')'
    ),
    ...
    tuple_struct_pattern: $ => seq(
      ...
      '(',
      field('patterns', seq(sepBy(',', $.pattern), optional(','))),
      ')'
    ),
    ...
    struct_pattern: $ => seq(
      ...
      '{',
        field('fields', sepBy(',', seq(choice($.field_pattern, $.remaining_field_pattern)),
        optional(','))),
      '}'
    ),
    ...
    identifier: ($) => choice($._primitive_identifier, $._raw_identifier, $.grit_metavariable),

    _raw_identifier: ($) => /r#[_\p{XID_Start}][_\p{XID_Continue}]*/,
